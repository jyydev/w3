...
async function fetchWithTimeout(url, options = {}, ms = sparkMarketFetchTimeoutMs) {
...
function buildSparkKnownMarketEntries(chain = "", savedCoin = "", savedCoinE = {}) {
  const knownMarket = getKnownSparkMarket(chain, {
    coin: savedCoin,
...
export async function getSparkAllMarkets({ chain = "" } = {}) {
  ...
  const knownMarkets = savedMarkets
    .flatMap(([savedCoin, savedCoinE]) =>
      buildSparkKnownMarketEntries(chain, savedCoin, savedCoinE),
    )
    .filter(Boolean);
